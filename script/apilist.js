...
function aa() {
    ...
    flag = 1;
    var url = 'https://www.juhe.cn/docs';
    var monitor = new Monitor(url, {
        ...
        path: {
            format: function(url, opt) {
                return opt.hostname + opt.pathname.replace(/\//g, '-');
            }
        }
    });
    ...
    monitor.capture(function(code) {
        ...
        console.log('case '+num+' done, success!\/n\/n');
        flag = 0;
        num++;
    });
...
var option = process.env.option;
var cal;
console.log(process.env);
if(option === 'start'){
    cal = setInterval(aa,1000);
}
